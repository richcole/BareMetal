...
ap_sleep:				; AP's will normally be running here
	...
	lodsd				; Load a 32-bit value. We only want the high 8 bits
	shr rax, 24			; Shift to the right and AL now holds the CPU's APIC ID
	mov r15b, al			; Save the APIC ID to R15B
	...
	; Clear registers. Gives us a clean slate to work with (only AL and RSP are set)
	xor rax, rax			; aka r0
	xor rcx, rcx			; aka r1
	xor rdx, rdx			; aka r2
	xor rbx, rbx			; aka r3
	...
	xor rsi, rsi			; aka r6
	xor rdi, rdi			; aka r7
	mov al, r15b			; Put the CPU APIC ID back into AL
	xor r8, r8
	xor r9, r9
	xor r10, r10
	xor r11, r11
	...
	; Calculate this AP's offset in the task list
	mov rsi, taskdata
...
continue:	
	...
	add rsi, 0x20			; Add 0x20 for the offset into the Local APIC ID
	lodsd				; Load a 32-bit value. We only want the high 8 bits
	shr rax, 24			; Shift to the right and AL now holds the CPU's APIC ID
